...
const cabTypes = [
  { name: 'Hatchback', seats: '4 Seats', price: '₹12/km', bgColor: 'bg-yellow-200' },
  { name: 'Sedan', seats: '4 Seats', price: '₹15/km', bgColor: 'bg-yellow-300' },
  { name: 'SUV', seats: '6-7 Seats', price: '₹20/km', bgColor: 'bg-yellow-400' },
  { name: 'Luxury', seats: '4 Seats', price: '₹35/km', bgColor: 'bg-yellow-500' },
];
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="grid lg:grid-cols-3 gap-8 mb-12">
                  {currentService.subServices.map((sub) => (
                    <Card key={sub.title} className="group hover:shadow-xl transition-all border-0 shadow-lg border border-gray-100">
                      <CardContent className="p-6">
                        <div className="w-14 h-14 bg-yellow-100 rounded-2xl flex items-center justify-center mb-5 group-hover:bg-yellow-400 transition-colors">
                          <sub.icon className="w-7 h-7 text-black" />
                        </div>
                        <h3 className="text-xl font-bold text-black mb-2">{sub.title}</h3>
                        <p className="text-gray-500">{sub.desc}</p>
                      </CardContent>
                    </Card>
                  ))}
                </div>
                  ...
                  <div className="bg-gray-50 rounded-3xl p-8 sm:p-12">
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                      {cabTypes.map((cab) => (
                        ...
                        >
                          <div className={`w-20 h-20 ${cab.bgColor} rounded-full mx-auto mb-4 flex items-center justify-center group-hover:scale-110 transition-transform`}>
                          ...
                          <p className="text-black font-semibold mt-2">{cab.price}</p>
                        </div>
                      ))}
                    </div>
                  </div>
